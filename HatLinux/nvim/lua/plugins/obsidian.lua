-- Windows-only plugin (uses cmd.exe for URL opening)
return {
	{
	...
	opts = {
		...
		-- Open URLs/images - platform specific
...

		{
			...
			function()
				if vim.fn.has("win32") == 1 or vim.fn.has("win64") == 1 then
					...
					vim.notify("obsidian:// URLs require the Obsidian app on Linux", vim.log.levels.WARN)
				end
			end,
