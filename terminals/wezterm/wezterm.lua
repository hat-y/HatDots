...
-- ===== Basic Settings =====
-- Linux shell: try zsh first, fallback to bash
local shell = os.execute("command -v zsh") and "zsh" or "bash"
config.default_prog = { shell }

-- Font configuration with Linux-friendly fallback
...
-- Try to load the font, fallback to system fonts on Linux
...
if not ok then
	...
	-- Linux fallback chain: JetBrains Mono → Fira Code → Source Code Pro
	wezterm.log_info("Falling back to JetBrains Mono")
	...
	-- Second fallback
	ok, font_err = pcall(function()
		return wezterm.font(font_family, font_options)
	end)
	if not ok then
		wezterm.log_info("Font '" .. font_family .. "' not found: " .. tostring(font_err))
		wezterm.log_info("Falling back to Fira Code")
		font_family = "Fira Code"
	end
end

config.font = wezterm.font(font_family, font_options)
-- Slightly smaller font size for Linux DPI (high-DPI displays common on Linux laptops)
config.font_size = 14
...
-- Use OpenGLES on Linux (more compatible than WebGPU which may not be available)
config.front_end = "OpenGLES"
...
-- ===== System Integration =====
-- Linux launch menu with zsh/bash options
config.launch_menu = {
	{
		label = "zsh",
		args = { "zsh", "-l" },
...
config.keys = {
	...
	-- Ctrl+h / Ctrl+l smart (same as Windows for familiarity)
	...
	-- Alt+h / Alt+l alternative (Linux convention - avoids Ctrl conflicts)
	{ key = "h", mods = "ALT", action = pass_to_vim_or(act.SwitchWorkspaceRelative(-1), "h", "ALT") },
	{ key = "l", mods = "ALT", action = pass_to_vim_or(act.SwitchWorkspaceRelative(1), "l", "ALT") },
	...
	-- Copy and paste (Linux/Unix convention)
